/**
 * Jogo de Futebol 2D - Versão Refatorada e Responsiva com Login
 */
...
class Game {
  constructor() {
    ...
    // Carregar dados do usuário do login
    this.loadUserData();
  ...
  loadUserData() {
    ...
    if (userData) {
      try {
        ...
        this.selectedTeam = data.team || 'red';
  ...
  initUI() {
    ...
    // Definir o time selecionado do login
    if (this.ui.team && this.selectedTeam) {
      this.ui.team.value = this.selectedTeam;
    }
  }
  ...
  initPlayers() {
    ...
    this.opponent = new Player(
      ...
      this.canvas.height / 2,
      this.selectedTeam === 'red' ? 'blue' : 'red',
  ...
  initEventListeners() {
    ...
    // Botão de logout
    if (this.ui.logout) {
      this.ui.logout.addEventListener('click', () => {
